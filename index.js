...
//Anonymous websocket, for getting general information. New races, pinging, and authentication.
	anon.on('connection', function(ws, req) {
		...
		try{ws.send(json.stringify({ok:true, code:3, msg:"HI_ANON"}));} catch (e) {logToSQL(e, ip);}
		ws.on('message', function(data) {
			try {
				d = json.parse(data);
			} catch (e) {
				try{ws.send(json.stringify({ok:false, code:-3, msg:"NOT_JSON5"}));} catch (e) {logToSQL(e, ip);}
				return;
			}

			switch (d["cmd"]) {
				case undefined:
					try{ws.send(json.stringify({ok:false, code:-1, msg:"NO_CMD"}));} catch (e) {logToSQL(e, ip);}
					return;
				case "species":
					connection.query("select * from spec", function(error,b) {
						if(error) logToSQL(error,  ip);
							try{ws.send(json.stringify({ok:true, code:4, data:b}));} catch (e) {logToSQL(e,  ip);}
						});
					return;
				...
				case "auth":
					if(d["data"] == undefined) {
						...
						data = d["data"];
						data.username = data.un.toLowerCase();
						data.password = data.pw;
						connection.query("select ce, password, token from users where username = ?", [data.username], function(error,b) {
							if (error) logToSQL(error, ip);
							if(b.length == 0) {try{ws.send(json.stringify({ok:false, code:-4, msg:"NOBODY_FOUND"}))} catch (e) {logToSQL(e, ip);}} else {
								s = b[0];
								if(s.ce !== "0") {try{ws.send(json.stringify({ok:false, code:-4, msg:"CONF_EMAIL"}));} catch (e) {logToSQL(e, ip);}}
								else if(password.verify(data.password, s.password) == false) {
									try{ws.send(json.stringify({ok:false, code:-4, msg:"INC_PASS"}));} catch (e) {logToSQL(e, ip);}
								} else {
									try{ws.send(json.stringify({ok:true, code:4, data:Buffer.from(s.token).toString('base64')}));} catch (e) {logToSQL(e, ip);}
								}
							}
						})
					}
				...
				if(d["data"] == undefined) {
					...
					data.username = data.un.toLowerCase();
					data.password = data.pw;
					connection.query("select uid, ce, password from users where username = ?", [data.username], function(error,b) {
						if (error) logToSQL(error, ip);
						if(b.length == 0) {try{ws.send(json.stringify({ok:false, code:-4, msg:"NOBODY_FOUND"}))} catch (e) {logToSQL(e, ip);}} else {
							s = b[0];
							if(s.ce !== "0") {try{ws.send(json.stringify({ok:false, code:-4, msg:"CONF_EMAIL"}));} catch (e) {logToSQL(e, ip);}}
								...
								try{ws.send(json.stringify({ok:true, code:4, data:{token: token, username:data.username}}));} catch (e) {logToSQL(e, ip);}
							}
						}
					});
				}
				...
				if(d["data"] == undefined) {
					...
					if(m["code"] == undefined || m["password"] == undefined || m["conpass"] == undefined) {
						try{ws.send(json.stringify({ok: false, code:-3, msg:"MISSING_DATA"}));} catch (e) {logToSQL(e, ip);}
						return
					}
					connection.query("select uid from users where rs = ?", [m["code"]], function(a, b) {
						if (error) logToSQL(error, ip);
						if (b.length != 1) {
							try{ws.send(json.stringify({ok:false, code:-4, msg:"FRAUD"}));} catch (e) {logToSQL(e, ip);}
							console.log(ip + " tried to access a code that doesn't exist.");
						} else if (m["password"] != m["conpass"]) {try{ws.send(json.stringify({ok: false, code: -4, msg: "DIFFERENT"}))} catch (e) {logToSQL(e, ip);}}
							else {
			...
			if(d["data"] == undefined) {
				...
				connection.query("select email from users where username = ?", [d["data"].toLowerCase()], function (a, b) {
					if (error) logToSQL(error, ip);
					if (b.length != 1) {try{ws.send(json.stringify({ok:false, code:-3, msg:"NO_USR"}))} catch (e) {logToSQL(e, ip);}}
					else {
						token = uuid(30);
						...
						try{ws.send(json.stringify({ok: true, code: 4, msg: "SENT_EMAIL"}));} catch (e) {logToSQL(e, ip);}
					}
				});
			}
				...
				if(d["data"] == undefined) {
					...
					try{ws.send(json.stringify({ok:false, code:-4, data:"BL"}))} catch (e) {logToSQL(e, ip);}
				} else {
					connection.query("select username from users where username = ?", [d["data"].toLowerCase()], function (a, b) {
						if (error) logToSQL(error, ip);
						if (b.length > 0) {try{ws.send(json.stringify({ok:true, code:4, data:"F"}));} catch (e) {logToSQL(e, ip);}}
						else {try{ws.send(json.stringify({ok:true, code:4, data:"NF"}));} catch (e) {logToSQL(e, ip);}}
					});
				}
					return;
				case "checkEmail":
				if(d["data"] == undefined) {
					...
					connection.query("select username from users where email = ?", [d["data"]], function (a, b) {
						if (error) logToSQL(error, ip);
						if (b.length > 0) {try{ws.send(json.stringify({ok:true, code:4, data:false}));} catch (e) {logToSQL(e, ip);}}
						else {try{ws.send(json.stringify({ok:true, code:4, data:true}));} catch (e) {logToSQL(e, ip);}}
					});
				}
				case "register":
				if(d["data"] == undefined) {
					...
					connection.query("select username from users where username = ? or email = ?", [n.username, n.email], function (a, b) {
						if(b.length > 0) {try{ws.send(json.stringify({ok: false, code:6, msg: "ACCT_EXISTS"}))} catch (e) {logToSQL(e, ip);}}
						else if (blacklist.includes(n.un.toLowerCase()))  {try{ws.send(json.stringify({ok: false, code:6, msg: "ACCT_BLACKLIST"}))} catch (e) {logToSQL(e, ip);}}
						else {
							token = uuid(30);
							confirmEmail = uuid(30);
							connection.query("INSERT INTO `users`(`username`, `password`, `email`, `token`, `ce`, `spid`) VALUES (?,?,?,?,?,?);", [n.username, password.hash(n.pw), n.email, token, confirmEmail, n.sp], function (error) {
								if (error) logToSQL(error, ip);
...
//Main Websocket, for processing most events related to the game
	main.on('connection', function connection(ws, req) {
		...
		if (config.get("user." + uid) == undefined) {
			try{ws.send('{code:1}');} catch (e) {logToSQL(e, ip);}
			ws.on('message', function incoming(data) {
				...
				if (typeof config.get("user." + uid) !== "undefined") {
					...
					switch (jsonData["cmd"]) {
						case undefined:
							try{ws.send(json.stringify({ ok: false, msg: "cmd_not_found", code: -1 }));} catch (e) {logToSQL(e, ip);}
							return;
					...
					if (jsonData["atoken"] == undefined) ws.close(1013)
						...
						connection.query("SELECT username, bal from users where token = ?", [jsonData["atoken"]], function asdf(a, b) {
							if (b.length == 1) {
								...
								try{ws.send(json.stringify({ ok: true, msg: "I_THOUGHT_I_REMEMBERED_YOU_OWO", code: 5 }));} catch (e) {logToSQL(e, ip);}
							} else ws.close(1013);
						});
					}
				}
			...
			try{ws.send('{"code":2}');} catch (e) {logToSQL(e, ip);}
		}
	});
...
//Battlesocket, for processing battling sequences
	battle.on('connection', function connection(ws, req) {
		...
		pppp = false;
		if(config.get("user."+uid+"battleid")) {try{ws.send(json.stringify({ok:true, code:2, bid: config.get("user."+uid+".battleid"), msg:"YOU_ARE_STILL_IN_A_FIGHT"}));} catch (e) {logToSQL(e, ip);}}
		else {
			r = uuid(7);
