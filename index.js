...
con.connect(function(err) {
	...
	anon.on('connection', function(ws, req) {
		let ip = req.headers['x-forwarded-for'];
		let uid = ip.replace(/\./g, '');
		ws.send(json.stringify({ok:true, code:3, msg:"HI_ANON"}), pdc(error, con, ip));
		ws.on('message', function(data) {
			try {
				d = json.parse(data);
			} catch (e) {
				ws.send(json.stringify({ok:false, code:-3, msg:"NOT_JSON5"}), pdc(error, con, ip));
				return;
			}
			if (d["cmd"] == undefined) {
				ws.send(json.stringify({ok:false, code:-1, msg:"NO_CMD"}), pdc(error, con, ip));
				return;
			} else if (d["cmd"] == "species") {
				con.query("select * from spec", function(a,b) {
					if(a) throw a;
					ws.send(json.stringify({ok:true, code:4, data:b}), pdc(error, con, ip));
				});
			} else if (d["cmd"] == "auth") {
				if(d["data"] == undefined) {
					ws.send(json.stringify({ok:false, code:-3, msg:"NO_DATA_FOUND"}), pdc(error, con, ip));
				} else {
					data = d["data"]
					//un, pw
					...
					con.query("select ce, password, token from users where username = ?", [data.un], function(a,b) {
						if (a) throw a;
						if(b.length == 0) {ws.send(json.stringify({ok:false, code:-4, msg:"NOBODY_FOUND"}), pdc(error, con, ip))} else {
							s = b[0];
							if(s.ce !== "0") {ws.send(json.stringify({ok:false, code:-4, msg:"CONF_EMAIL"}), pdc(error, con, ip));}
							else if(pass.verify(data.pw, s.password) == false) {
								ws.send(json.stringify({ok:false, code:-4, msg:"INC_PASS"}), pdc(error, con, ip));
							} else {
								ws.send(json.stringify({ok:true, code:4, data:Buffer.from(s.token).toString('base64')}), pdc(error, con, ip));
							}
						}
					})
				}
				...
				if(d["data"] == undefined) {
					...
					if(m["code"] == undefined || m["password"] == undefined || m["conpass"] == undefined) {
						ws.send(json.stringify({ok: false, code:-3, msg:"MISSING_DATA"}), pdc(error, con, ip));
						return
					}
					con.query("select uid from users where rs = ?", [m["code"]], function(a, b) {
						if (a) throw a;
						if (b.length != 1) {
							ws.send(json.stringify({ok:false, code:-4, msg:"FRAUD"}), pdc(error, con, ip));
							console.log(ip + " tried to access a code that doesn't exist.");
						} else {
							if (m["password"] != m["conpass"]) {ws.send(json.stringify({ok: false, code: -4, msg: "DIFFERENT"}), pdc(error, con, ip))}
							else {
								con.query("UPDATE users set password = ?, token = ?, rs = 0 where rs = ?", [pass.hash(m["password"]), uuid(30), m["code"]], function(a, b) {
									if (a) throw a;
									ws.send(json.stringify({ok:true, code:4, data:"LOGIN_AGAIN"}), pdc(error, con, ip));
								});
							}
						}
					});
				...
				if(d["data"] == undefined) {
					...
					con.query("select email from users where username = ?", [d["data"].toLowerCase()], function (a, b) {
						if (a) throw a;
						if (b.length != 1) {ws.send(json.stringify({ok:false, code:-3, msg:"NO_USR"}), pdc(error, con, ip))}
						else {
							token = uuid(30);
							...
							ws.send(json.stringify({ok: true, code: 4, msg: "SENT_EMAIL"}), pdc(error, con, ip));
						}
					});
				}
			} else if (d["cmd"] == "cun") {
				if(d["data"] == undefined) {
					...
					ws.send(json.stringify({ok:false, code:-4, data:"BL"}), pdc(error, con, ip))
				} else {
					con.query("select username from users where username = ?", [d["data"].toLowerCase()], function (a, b) {
						if (a) throw a;
						if (b.length > 0) {ws.send(json.stringify({ok:true, code:4, data:"F"}), pdc(error, con, ip));}
						else {ws.send(json.stringify({ok:true, code:4, data:"NF"}), pdc(error, con, ip));}
					});
				}
			} else if (d["cmd"] == "cem") {
				if(d["data"] == undefined) {
					...
					con.query("select username from users where email = ?", [d["data"]], function (a, b) {
						if (a) throw a;
						if (b.length > 0) {ws.send(json.stringify({ok:true, code:4, data:false}), pdc(error, con, ip));}
						else {ws.send(json.stringify({ok:true, code:4, data:true}), pdc(error, con, ip));}
					});
				}
				...
				if(d["data"] == undefined) {
					...
					con.query("select username from users where username = ? or email = ?", [n.un, n.em], function (a, b) {
						if(b.length > 0) {ws.send(json.stringify({ok: false, code:6, msg: "ACCT_EXISTS"}), pdc(error, con, ip))}
						else if (black.includes(n.un.toLowerCase()))  {ws.send(json.stringify({ok: false, code:6, msg: "ACCT_BLACKLIST"}), pdc(error, con, ip))}
						else {
							token = uuid(30);
							ce = uuid(30);
							con.query("INSERT INTO `users`(`username`, `password`, `email`, `token`, `ce`, `spid`) VALUES (?,?,?,?,?,?);", [n.un, pass.hash(n.pw),n.em, token, ce, n.sp], function (a) {
								...
								ws.send(json.stringify({ok:true, code:4, msg:"CHECK_EMAIL"}), pdc(error, con, ip));
							});
						}
					});
				}
		...
		if (cfg.get("user." + uid) == undefined) {
			...
			ws.on('message', function incoming(data) {
				...
				if (typeof cfg.get("user." + uid) !== "undefined") {
					...
					if (x["cmd"] == undefined) {
						ws.send(json.stringify({ ok: false, msg: "cmd_not_found", code: -1 }), pdc(error, con, ip));
						return;
					} else {
						if (x["cmd"] == "ddm") {
					...
					if (x["atoken"] == undefined) ws.close(1013)
						...
						con.query("SELECT username, bal from users where token = ?", [x["atoken"]], function asdf(a, b) {
							if (b.length == 1) {
								...
								ws.send(json.stringify({ ok: true, msg: "I_THOUGHT_I_REMEMBERED_YOU_OWO", code: 5 }), pdc(error, con, ip));
							} else ws.close(1013);
						});
					}
				}
	...
	chat.on('connection', function connection(ws, req) {
		...
		ws.on('message', function msg(data) {
			...
			if (data.length >= 80) {
				ws.send(json.stringify({ ok: false, display: "*Your voice falls on deaf ears. (Too many characters.)", color: "red" }), pdc(error, con, ip));
				ds = 1
			}
			if (ds == 0) {
				if (data.split(" ")[0] == "!s") {
					//Use shout
					ws.send(json.stringify({ ok: false, display: "*Shouts are not setup yet.", color: "red" }), pdc(error, con, ip));
										...
										if (client.readyState === WebSocket.OPEN) {
											if (cfg.get("user." + client._socket.remoteAddress.replace(/::ffff:/g, '').replace(/\./g, '') + ".token") == h.token) {
												con.query("SELECT citid from users where token = ?", [cfg.get("user." + client._socket.remoteAddress.replace(/::ffff:/g, '').replace(/\./g, '') + ".token")], function a(e, f) {
													...
													if (e) throw e;
													if (f.length == 1) { client.send(json.stringify({ ok: true, display: cfg.get("user." + uid + ".un") + ">> " + data.replace("!f all ", ""), color: "pink" }), pdc(error, con, ip)); }
												});
											}
										}
									});
								});
								ws.send(json.stringify({ ok: true, display: cfg.get("user." + uid + ".un") + ">> " + data.replace("!f all ", ""), color: "pink" }), pdc(error, con, ip));
							});
						});
					} else {
						fun = data.split(" ")[1].toLowerCase();
						if (fun == cfg.get("user." + uid + ".un")) { ws.send(json.stringify({ ok: false, display: "You cannot send a message to yourself." }), pdc(error, con, ip)) } else {
							...
							con.query("select uid from users where token = ?", [cfg.get("user." + uid + ".token")], function(f, g) {
								con.query("select token from users where username = ? and uid in ( select ut from friends where uf = ? ) or uid in ( select uf from friends where ut = ? )", [fun, g[0].uid, g[0].uid], function(a, b) {
									if (b.length == 0) { ws.send(json.stringify({ ok: false, display: fun + " is not a friend of yours. Are you sure you typed their name correctly?" }), pdc(error, con, ip)) } else {
										chat.clients.forEach(function each(client) {
											if (client.readyState === WebSocket.OPEN) {
												con.query("SELECT citid from users where token = ?", [cfg.get("user." + client._socket.remoteAddress.replace(/::ffff:/g, '').replace(/\./g, '') + ".token")], function a(a, e) {
													...
													if (a) throw a;
													if (b.length == 1) {
														client.send(json.stringify({ ok: true, display: cfg.get("user." + uid + ".un") + ">> " + data.replace("!f " + fun + " ", ""), color: "pink" }), pdc(error, con, ip));
														ws.send(json.stringify({ ok: true, display: cfg.get("user." + uid + ".un") + ">> " + data.replace("!f " + fun + " ", ""), color: "pink" }), pdc(error, con, ip));
													}
												});
											}
										});
					...
					chat.clients.forEach(function each(client) {
						if (client.readyState === WebSocket.OPEN) {
							con.query("SELECT citid from users where token = ?", [cfg.get("user." + client._socket.remoteAddress.replace(/::ffff:/g, '').replace(/\./g, '') + ".token")], function a(a, b) {
								uid = req.connection.remoteAddress.replace(/::ffff:/g, '').replace(/\./g, '');
								if (a) throw a;
								if (b.length == 1) { client.send(json.stringify({ ok: true, display: cfg.get("user." + uid + ".un") + ">> " + data }), pdc(error, con, ip)); }
							});
						}
					});
				}
	...
	battle.on('connection', function connection(ws, req) {
		isconnected(req, ws);
		if(cnf.get("user."+uid+"battleid")) {ws.send(json.stringify({ok:true, code:2, bid: cnf.get("user."+uid+"battleid"), msg:"YOU_ARE_STILL_IN_A_FIGHT"}), pdc(error, con, ip));}
		else {
